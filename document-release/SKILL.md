...
description: |
  ...
  diff, updates README/ARCHITECTURE/CONTRIBUTING/CLAUDE.md (or GEMINI.md / .cursorrules) to match what shipped,
...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.g-stack-gemini/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.g-stack-gemini/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not g-stack-gemini-specific):
...
**CLAUDE.md (or GEMINI.md / .cursorrules) / project instructions:**
...
1. Does the README's feature/capability list match what CLAUDE.md (or GEMINI.md / .cursorrules) (or project instructions) describes?
2. Does ARCHITECTURE's component list match CONTRIBUTING's project structure description?
3. Does CHANGELOG's latest version match the VERSION file?
4. **Discoverability:** Is every documentation file reachable from README.md or CLAUDE.md (or GEMINI.md / .cursorrules)? If
   ARCHITECTURE.md exists but neither README nor CLAUDE.md (or GEMINI.md / .cursorrules) links to it, flag it. Every doc
...
```bash
gh pr view --json body -q .body > /tmp/g-stack-gemini-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/g-stack-gemini-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/g-stack-gemini-pr-body-$$.md
```
...
- **Discoverability matters.** Every doc file should be reachable from README or CLAUDE.md (or GEMINI.md / .cursorrules).
