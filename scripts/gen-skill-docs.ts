...
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.g-stack-gemini/analytics/skill-usage.jsonl 2>/dev/null || true
\`\`\`
...
**Local:** \`.g-stack-gemini/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
source <(~/.agents/skills/g-stack-gemini/bin/g-stack-gemini-slug 2>/dev/null) && mkdir -p ~/.g-stack-gemini/projects/$SLUG
\`\`\`
Write to: \`~/.g-stack-gemini/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
function findTemplates(): string[] {
  ...
  const candidates = [
    ...
    path.join(ROOT, 'g-stack-gemini-upgrade', 'SKILL.md.tmpl'),
