...
const PARTIAL_PATH = path.join(G_STACK_GEMINI_DEV_DIR, 'evals', '_partial-e2e.json');
...
export function renderDashboard(heartbeat: HeartbeatData | null, partial: PartialData | null): string {
  ...
  if (heartbeat?.runId) {
    const logPath = path.join(G_STACK_GEMINI_DEV_DIR, 'e2e-runs', heartbeat.runId, 'progress.log');
