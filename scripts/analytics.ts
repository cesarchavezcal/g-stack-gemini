#!/usr/bin/env bun
/**
 * analytics — CLI for viewing g-stack-gemini skill usage statistics.
 *
 * Reads ~/.g-stack-gemini/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.g-stack-gemini', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('g-stack-gemini skill usage analytics');
