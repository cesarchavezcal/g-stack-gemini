/**
 * the AI Agent subprocess runner for skill E2E testing.
 *
 * Spawns the AI Agent as a completely independent process (not via Agent SDK),
 * so it works inside the AI Agent sessions. Pipes prompt via stdin, streams
...
const G_STACK_GEMINI_DEV_DIR = path.join(os.homedir(), '.g-stack-gemini-dev');
const HEARTBEAT_PATH = path.join(G_STACK_GEMINI_DEV_DIR, 'e2e-live.json');
...
export async function runSkillTest(options: {
  ...
  if (runId) {
    try {
      runDir = path.join(G_STACK_GEMINI_DEV_DIR, 'e2e-runs', runId);
      fs.mkdirSync(runDir, { recursive: true });
    } catch { /* non-fatal */ }
  }

  // Spawn the AI Agent with streaming NDJSON output. Prompt piped via stdin to
  ...
  const proc = Bun.spawn(['sh', '-c', `cat "${promptFile}" | ${agentBin} ${args.map(a => `"${a}"`).join(' ')}`], {
  ...
  if (resultLine) {
    if (resultLine.is_error) {
      // the AI Agent can return subtype=success with is_error=true (e.g. API connection failure)
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.g-stack-gemini', 'test-transcripts');
