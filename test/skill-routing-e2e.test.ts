...
/** Copy all SKILL.md files into tmpDir/.agents/skills/g-stack-gemini/ for auto-discovery */
function installSkills(tmpDir: string) {
  const skillDirs = [
    '', // root g-stack-gemini SKILL.md
    ...
    'g-stack-gemini-upgrade', 'humanizer',
  ];
  ...
  for (const skill of skillDirs) {
    ...
    const destDir = skill
      ? path.join(tmpDir, '.gemini', 'skills', 'g-stack-gemini', skill)
      : path.join(tmpDir, '.gemini', 'skills', 'g-stack-gemini');
