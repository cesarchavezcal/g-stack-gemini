 ...
 *   2. git rev-parse --show-toplevel → projectDir/.g-stack-gemini/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .g-stack-gemini/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.g-stack-gemini');
...
/**
 * Create the .g-stack-gemini/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .g-stack-gemini/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.g-stack-gemini\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.g-stack-gemini/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
