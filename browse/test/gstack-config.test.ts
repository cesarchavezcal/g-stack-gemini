/**
 * Tests for bin/g-stack-gemini-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * G_STACK_GEMINI_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'g-stack-gemini-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'g-stack-gemini-config-test-'));
});
...
describe('g-stack-gemini-config', () => {
  // ─── get ──────────────────────────────────────────────────
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { G_STACK_GEMINI_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
