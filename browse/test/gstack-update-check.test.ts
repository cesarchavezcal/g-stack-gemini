/**
 * Tests for bin/g-stack-gemini-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * G_STACK_GEMINI_DIR / G_STACK_GEMINI_STATE_DIR / G_STACK_GEMINI_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'g-stack-gemini-update-check');

let g-stack-geminiDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      G_STACK_GEMINI_DIR: g-stack-geminiDir,
      G_STACK_GEMINI_STATE_DIR: stateDir,
      G_STACK_GEMINI_REMOTE_URL: `file://${join(g-stack-geminiDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  g-stack-geminiDir = mkdtempSync(join(tmpdir(), 'g-stack-gemini-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'g-stack-gemini-state-test-'));
  // Link real g-stack-gemini-config so update_check config check works
  const binDir = join(g-stack-geminiDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'g-stack-gemini-config'), join(binDir, 'g-stack-gemini-config'));
});

afterEach(() => {
  rmSync(g-stack-geminiDir, { recursive: true, force: true });
...
describe('g-stack-gemini-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(g-stack-geminiDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(g-stack-geminiDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(g-stack-geminiDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(g-stack-geminiDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      G_STACK_GEMINI_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ G_STACK_GEMINI_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(g-stack-geminiDir, 'VERSION'), version + '\n');
