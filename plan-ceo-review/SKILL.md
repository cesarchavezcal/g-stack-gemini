...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.g-stack-gemini/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```
Then read CLAUDE.md (or GEMINI.md / .cursorrules), TODOS.md, and any existing architecture docs.

**Design doc check:**
```bash
SLUG=$(~/.agents/skills/g-stack-gemini/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.g-stack-gemini/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.g-stack-gemini/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
source <(~/.agents/skills/g-stack-gemini/bin/g-stack-gemini-slug 2>/dev/null) && mkdir -p ~/.g-stack-gemini/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.g-stack-gemini/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.g-stack-gemini/projects/$SLUG/ceo-plans/{old-plan}.md ~/.g-stack-gemini/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.g-stack-gemini/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With CC + g-stack-gemini,
...
For LLM/prompt changes: Check CLAUDE.md (or GEMINI.md / .cursorrules) for the "Prompt/LLM changes" file patterns. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against.
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.agents/skills/review/TODOS-format.md`.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+g-stack-gemini: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
~/.agents/skills/g-stack-gemini/bin/g-stack-gemini-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.g-stack-gemini/projects/` only (local, personal reference)
- **C)** Skip
