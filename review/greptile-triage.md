...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.agents/skills/g-stack-gemini/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.g-stack-gemini/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior g-stack-gemini reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior g-stack-gemini reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains g-stack-gemini markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain g-stack-gemini markers.

3. **If a prior g-stack-gemini reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior g-stack-gemini reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.agents/skills/g-stack-gemini/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.g-stack-gemini/projects/$REMOTE_SLUG"
mkdir -p ~/.g-stack-gemini
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.g-stack-gemini/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.g-stack-gemini/greptile-history.md` (global aggregate)
