...
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.g-stack-gemini/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
ls ~/.g-stack-gemini/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Update CLAUDE.md (or GEMINI.md / .cursorrules)** (or create it if it doesn't exist) — append this section:
...
- A) Ship it — write DESIGN.md and CLAUDE.md (or GEMINI.md / .cursorrules)
