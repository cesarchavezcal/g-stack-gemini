...
echo '{"skill":"qa-only","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.g-stack-gemini/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
| Mode | full | `--quick`, `--regression .g-stack-gemini/qa-reports/baseline.json` |
...
```bash
REPORT_DIR=".g-stack-gemini/qa-reports"
