# Contributing to g-stack-gemini

Thanks for wanting to make g-stack-gemini better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

g-stack-gemini skills are Markdown files that Gemini CLI discovers from a `skills/` directory. Normally they live at `~/.agents/skills/g-stack-gemini/` (your global install). But when you're developing g-stack-gemini itself, you want Gemini CLI to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.agents/skills/` directory so Gemini CLI reads skills straight from your checkout.

```bash
git clone <repo> && cd g-stack-gemini
...
Contributor mode turns g-stack-gemini into a self-improving tool. Enable it and Gemini CLI
will periodically reflect on its g-stack-gemini experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.g-stack-gemini/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.agents/skills/g-stack-gemini/bin/g-stack-gemini-config set g_stack_gemini_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork g-stack-gemini, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use g-stack-gemini normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.g-stack-gemini/contributor-logs/`
3. **Fork and clone g-stack-gemini** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where g-stack-gemini annoyed you)
   ln -sfn /path/to/your/g-stack-gemini-fork .agents/skills/g-stack-gemini
   cd .agents/skills/g-stack-gemini && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using g-stack-gemini** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix g-stack-gemini while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ g-stack-gemini sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all 15 skills.

## Working on g-stack-gemini inside the g-stack-gemini repo

When you're editing g-stack-gemini skills and want to test them by actually using g-stack-gemini
in the same repo, `bin/dev-setup` wires this up. It creates `.agents/skills/`
symlinks (gitignored) pointing back to your working tree, so Gemini CLI uses
your local edits instead of the global install.

```
g-stack-gemini/                          <- your working tree
├── .agents/skills/              <- created by dev-setup (gitignored)
│   ├── g-stack-gemini -> ../../         <- symlink back to repo root
│   ├── review -> g-stack-gemini/review
│   ├── ship -> g-stack-gemini/ship
...
cp .env.example .env
# Edit .env → set GOOGLE_API_KEY=sk-ant-...
...
When E2E tests run, they produce machine-readable artifacts in `~/.g-stack-gemini-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.g-stack-gemini-dev/` for post-mortem debugging and trend analysis.
...
```bash
# Needs GOOGLE_API_KEY in .env — included in bun run test:evals
```

- Uses `gemini-sonnet-4-6` for scoring stability
- Tests live in `test/skill-llm-eval.test.ts`
- Calls the Google API directly (not `gemini -p`), so it works from anywhere including inside Gemini CLI
...
For template authoring best practices (natural language over bash-isms, dynamic branch detection, `{{BASE_BRANCH_DETECT}}` usage), see GEMINI.md's "Writing SKILL templates" section.
...
**First-time setup:** Put your `GOOGLE_API_KEY` in `.env` in the main repo (see `.env.example`). Every Conductor workspace inherits it automatically.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.agents/skills/g-stack-gemini`. `bin/dev-teardown` restores the global one.
- **Conductor workspaces are independent.** Each workspace is its own git worktree. `bin/dev-setup` runs automatically via `conductor.json`.
- **`.env` propagates across worktrees.** Set it once in the main repo, all Conductor workspaces get it.
- **`.agents/skills/` is gitignored.** The symlinks never get committed.

## Testing your changes in a real project

**This is the recommended way to develop g-stack-gemini.** Symlink your g-stack-gemini checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/g-stack-gemini-checkout .agents/skills/g-stack-gemini
cd .agents/skills/g-stack-gemini && bun install && bun run build
```

Now every g-stack-gemini skill invocation in this project uses your working tree. Edit a
...
```bash
rm .agents/skills/g-stack-gemini
```

Gemini CLI falls back to `~/.agents/skills/g-stack-gemini/` automatically.
